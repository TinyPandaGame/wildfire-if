...
> **用途**：给生图 agent / GPT-Image-2 / Midjourney / 即梦 等模型生成沈夜的角色定型表（三视图）。
> **输出格式**：单张图 1920×1080（16:9），横向并排三视图（front/side/back）。
> **输出路径**：候选丢 `assets/art/characters/沈夜/sheet-candidates/`，定稿放 `sheet.png` + `ref.png`。
>
...
```
【角色定型表 — 沈夜（Shen Ye）— 三视图】

主体：
15 岁中国南方港口少年。身高 173cm，偏瘦但爆发力强，劳动型爆发体态
（不是健身房肌肉）。来自一个名叫"铁港"的衰败工业海港小镇，
父亲已故，自己在码头搬货谋生 2 年。
...
五官：
眉骨硬朗，眼神锐利冷硬，鼻梁挺直，薄嘴唇。
表情基线为不笑，沉默警觉，眼神是被生活磨硬过早的劳动者眼神。
...
服装：
洗得发白起球的灰色旧背心，廉价深色长裤，旧球鞋（右脚趾处有破洞）。

标志性细节：
前臂多处旧伤划痕，手掌新裂口（搬货留下，结痂中），手指有裂口。
劳动者的手——粗糙、有茧、不修指甲。
...
姿势：自然站立，双臂略微外展（不要垂直贴身），便于看清身材外形。

【背景】
纯净的浅灰中性背景，无场景，无道具，无地面阴影。
柔和均匀光照，无戏剧光影对比。
（这是用作 reference 图的，必须中性，便于后续 shot 生成时引用）

【风格】
写实电影感（cinematic realism），高细节度，照片级写实质感。
粗粝街头质感（gritty street texture）。

【NEGATIVE — 严禁出现】
gym muscle, anime style, cartoon, K-pop idol styling,
...
extra characters, deformed hands, multiple fingers errors,
face drift between the three views (the same person across all 3)
```
...
[STYLE]
Cinematic realism, high detail, photographic realism, gritty street texture.

[NEGATIVE]
gym muscle, anime style, cartoon, K-pop idol styling, smiling face,
polished glamour, clean ironed clothing, model-like beauty,
multiple separate characters, dramatic shadows, any scene background,
any environment, 3/4 angle view (must be STRICTLY front / side / back),
...
2. 跑 3-5 次（同 prompt 不同 seed）
3. 候选 .png 文件保存到 `assets/art/characters/沈夜/sheet-candidates/`
   命名：`YYYY-MM-DD-attempt-NN.png`
4. commit + push 到 wildfire-if main
5. 用户审核选定 → 复制为 `sheet.png` 和 `ref.png`
6. 后续含沈夜的 shot 生成都引用 `ref.png` 作为参考图
